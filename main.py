...
def get_default_car_data():
    """Генерирует шаблон данных для новой машины."""
...
def recalculate_auto_daily_mileage(car_profile):
    ...
    # ИСПРАВЛЕНО: берем первый элемент списка через [0]
...
def show_car_odometer_history_dialog(
    ...
    def render_history_list():
        ...
        for item in sorted_hist:
            ...
            # ИСПРАВЛЕНО: ft.border.all -> ft.Border.all (Flet 1.0+)
            history_container.controls.append(
                ft.Container(
                    content=ft.Row(
                        [
...
# Фрагмент №8: Окно истории общего пробега — Функции Добавления / Изменения / Удаления
    ...
    # ИСПРАВЛЕНО: ft.ElevatedButton -> ft.Button (Flet 1.0+)
...
# Фрагмент №9: Generator списка плиток регламентных работ
...
def build_maintenance_list(
    ...
    """Сборка списка регламентных работ."""
    ...
    for task, info in list(
        car_profile["maintenance_data"].items()
    ):
        ...
        if remaining_km <= 0:
            ...
            status_text = (
                f"В норме. Осталось: {remaining_km} км "
                f"(~{fc})"
            )
        ...
        has_h = any(
            h["task"] == task
            for h in car_profile.get("history", [])
        )
        
        # ОПРЕДЕЛЯЕМ ТЕКСТ ПОСЛЕДНЕГО ТО БЕЗ УРЕЗАНИЯ ИНТЕРФЕЙСА
        if has_h:
            last_text = (
                f"Последнее ТО: {info['last_service']} км "
                f"({info['date']})"
            )
        else:
            last_text = "Последнее ТО: данных нет"
        ...
        def make_reset(t=task, p=car_profile):
            def reset_click(_):
                ...
                p["maintenance_data"][t].update(
                    {"last_service": current_km, "date": now_str}
                )
                p["history"].append(
                    {
                        "task": t,
        ...
        def make_history(t=task, p=car_profile):
            return lambda _: show_task_history_dialog(
                page, t, p
            )

        def make_add(t=task, p=car_profile):
            return lambda _: show_add_task_history_dialog(
                page,
                db_data,
                t,
                p,
        ...
        sub_info = (
            f"{last_text} | "
            f"Регламент: {info['interval']} км"
        )
        
        # ИСПРАВЛЕНО: Кнопки теперь рендерятся ВСЕГДА, без условий
        tile = ft.ExpansionTile(
            ...
            subtitle=ft.Text(
                status_text, size=12, color=status_color
            ),
            bgcolor=bg_color,
