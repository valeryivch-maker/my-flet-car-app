...
try:
 ...
 try:
  import network_template as network
...
def main(page: ft.Page):
 ...
 def rebuild_ui():
  ...
  hist_text = "Истори пробега: " + " ".join([f"{h['value']} км ({h['date']})" for h in car_profile.get("odometer_history", [])[-2:]]) if car_profile.get("odometer_history") else "Истори пробега пуста"
  header_card = ft.Card(content=ft.Container(content=ft.Column([action_panel, ft.Divider(height=5), ft.Text("Обновление данных пробега", size=16, weight=ft.FontWeight.BOLD), ft.Column([current_odo_input, daily_input], expand=False, spacing=8), ft.Text(hist_text, size=11, color=ft.Colors.GREY_600, italic=True), ft.Column([ft.ElevatedButton("Обновит пробег и прогноз", on_click=update_forecast_click, height=45, bgcolor=ft.Colors.AMBER_700, color=ft.Colors.WHITE), ft.ElevatedButton("Истори пробега", on_click=lambda _: views.show_car_odometer_history_dialog(page, current_db, car_profile, rebuild_ui, show_message), height=45)], horizontal_alignment=ft.CrossAxisAlignment.STRETCH, spacing=10), ft.Text("Учет расходов на топливо", size=14, weight=ft.FontWeight.BOLD), ft.Row([ft.ElevatedButton("Заправит авто", icon=ft.Icons.LOCAL_GAS_STATION, bgcolor=ft.Colors.AMBER_700, color=ft.Colors.WHITE, on_click=lambda _: views.show_add_fuel_dialog(page, current_db, car_profile, lambda: page.data["refresh_ui"](), show_message), expand=True, height=40), ft.ElevatedButton("Журнал заправок", icon=ft.Icons.LIST_ALT, on_click=lambda _: views.show_fuel_history_dialog(page, current_db, car_profile, lambda: page.data["refresh_ui"](), show_message), expand=True, height=40)], spacing=10), ft.ElevatedButton("Журнал ремонтов", icon=ft.Icons.BUILD_CIRCLE, bgcolor=ft.Colors.BLUE_GREY_700, color=ft.Colors.WHITE, on_click=lambda _: views.show_repair_history_dialog(page, current_db, car_profile, lambda: page.data["refresh_ui"](), show_message), expand=True, height=40)], spacing=12), padding=12))
  if engine.app_state.get("view_mode") == "analytics": main_layout = ft.Column([header_card, views.generate_analytics_view(page, car_profile)], expand=False, scroll=ft.ScrollMode.AUTO)
 ...
 try:
  def start_worker():
   ...
   if c_data and "cars" in c_data and sc in c_data["cars"]:
    net_mod = sys.modules.get("network", __import__("network"))
    if hasattr(net_mod, "check_and_send_alerts"): net_mod.check_and_send_alerts(c_data["cars"][sc], car_name=sc)
  threading.Thread(target=start_worker, daemon=True).start()
 except Exception as e: print(f"Ошибка запуска алертов: {e}")
