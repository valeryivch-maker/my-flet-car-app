            ...
            content=ft.Column([
                ...
                ft.Row([
                    ...
                    ft.Text(f"{last_econ} грн" if last_econ < 0 else f"+{last_econ} грн", size=13,
                            weight=ft.FontWeight.BOLD, color=ft.Colors.GREEN_700 if last_econ >= 0 else ft.Colors.RED_600)
                ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
                ft.Column([
                    ft.Row([
...
def show_task_history_dialog(page, db_data, task_name, car_profile, rebuild, show_msg):
    ...
    def refresh():
        ...
        if not t_hist:
            ...
            for rec in sorted(t_hist, key=lambda x: int(x.get('odometer', 0)), reverse=True):
                def make_del(r=rec):
                    return lambda: [car_profile["history"].remove(r), engine.save_data(db_data), refresh(), rebuild(), show_msg("Удалено")]
                ...
                h_col.controls.append(ft.Container(
                    content=ft.Row([
                        ft.Column([
                            ...
                            ft.Text(f"{rec.get('part_name','')}" + (f" [Арт: {rec.get('part_code')}]" if rec.get('part_code') else ""), size=12, color=ft.Colors.GREEN_800, weight=ft.FontWeight.W_500),
    ...
    dlg = ft.AlertDialog(
        ...
        actions=[ft.TextButton("Закрыть", on_click=lambda e: [setattr(dlg, "open", False), page.update()])]
    )
...
def create_task_actions(page, db_data, p, t, current_km, rebuild, show_msg):
    def reset_click(__):
        ...
        engine.save_data(db_data)
        rebuild()
        show_msg("ТО отмечено как выполненное!")
    ...
    def change_click():
        ...
        def save(_):
            try:
                ...
                old = p["maintenance_data"].pop(t)
                old["interval"] = ni
                p["maintenance_data"][nn] = old
                if nn != t:
                    if "history" not in p: p["history"] = []
                ...
                show_msg("Ошибка заполнения")
                
        dlg = ft.AlertDialog(
            title=ft.Text("Правка регламента"), 
            content=ft.Column([n_in, i_in], tight=True),
            actions=[ft.TextButton("OK", on_click=save)]
        )
        page.overlay.append(dlg)
        dlg.open = True
        page.update()
    ...
    def delete_click(_):
        ...
        engine.save_data(db_data)
        rebuild()
        show_msg("Регламент полностью удален")
...
def build_maintenance_list(page, db_data, car_name, car_profile, header_card, rebuild, show_msg, add_task_fn=None):
    ...
    status_header = ft.Row([
        ...
        ft.IconButton(ft.Icons.ADD_CIRCLE, tooltip="Создать новый регламент TO", icon_color=ft.Colors.BLUE_600, on_click=add_task_fn)
    ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN)
                    ...
                    ft.Container(
                        content=ft.Column([
                            ...
                            ft.Row([
                                ...
                                ft.IconButton(ft.Icons.SETTINGS, tooltip="Настройки регламента", icon_color=ft.Colors.BLUE_GREY_600, on_click=c_fn),
                                ft.IconButton(ft.Icons.DELETE_FOREVER, tooltip="Полностью удалить регламент", icon_color=ft.Colors.RED_400, on_click=d_fn)
...
def show_car_odometer_history_dialog(page, db_data, car_profile, rebuild, show_msg):
    ...
    def render():
        ...
        for item in sorted(car_profile.get("odometer_history", []), key=lambda x: int(x.get("value", 0)), reverse=True):
            def make_del(i=item):
                return lambda: [car_profile["odometer_history"].remove(i), engine.save_data(db_data), render(), rebuild(), show_msg("Удалено")]
            h_cont.controls.append(ft.Container(
                ...
                padding=5, border=ft.Border.all(1, ft.Colors.BLACK_12), border_radius=6
            ))
        if dlg: dlg.update()
        else: page.update()
    ...
    def refresh():
        ...
        if not f_hist:
            ...
            for rec in sorted(f_hist, key=lambda x: int(x.get("odometer", 0)), reverse=True):
                def make_del(r=rec):
                    return lambda: [car_profile["fuel_history"].remove(r), engine.save_data(db_data), refresh(), rebuild(), show_msg("Запись удалена")]
...
def show_add_fuel_dialog(page, db_data, car_profile, rebuild, show_msg):
    ...
    dlg = ft.AlertDialog(
        title=ft.Text("Учет заправки (до полного)"),
        content=ft.Column([
...
def show_repair_history_dialog(page, db_data, car_profile, rebuild, show_msg):
    ...
    search_field = ft.TextField(label="Поиск по журналу...", prefix_icon=ft.Icons.SEARCH, text_size=12, height=48, content_padding=ft.Padding(10, 0, 10, 0))
    dlg = None
    ...
    def refresh():
        ...
        if not filtered_hist:
            ...
            for rec in sorted(filtered_hist, key=lambda x: int(x.get("odometer", 0)), reverse=True):
                ...
                def make_edit(r=rec):
                    def open_edit_repair_dialog(_):
                        ...
                        edit_cat = ft.Dropdown(
                            label="Категория (Тег)", value=r.get("category", "Ходовая"),
                            options=[
                                ft.dropdown.Option("Двигатель"), ft.dropdown.Option("Ходовая"),
                                ft.dropdown.Option("Электрика"), ft.dropdown.Option("Кузов"),
                                ft.dropdown.Option("Салон"), ft.dropdown.Option("Расходники / Другое")
                            ]
                        )
                        ...
                        edit_rep_dlg = ft.AlertDialog(
                            adaptive=True, title=ft.Text("Правка записи ремонта"),
                ...
                cat_colors = {
                    "Двигатель": ft.Colors.RED_700, "Ходовая": ft.Colors.BLUE_700,
                    "Электрика": ft.Colors.ORANGE_800, "Кузов": ft.Colors.PURPLE_700,
                    "Салон": ft.Colors.BROWN_600, "Расходники / Другое": ft.Colors.BLUE_GREY_600
                }
                ...
                category_tag = ft.Container(
                    ...
                    bgcolor=tag_color, padding=ft.Padding(4, 2, 4, 2), border_radius=4
                )
                ...
                parts_row = ft.Row([
                    ft.Text(f" {rec.get('part_name')} [Арт: {rec.get('part_code')}]", size=12, color=ft.Colors.GREEN_800, expand=True),
    ...
    def add_repair_click(_):
        ...
        in_cat = ft.Dropdown(
            label="Категория (Тег ремонтов)", value="Ходовая",
            options=[
                ft.dropdown.Option("Двигатель"), ft.dropdown.Option("Ходовая"),
                ft.dropdown.Option("Электрика"), ft.dropdown.Option("Кузов"),
                ft.dropdown.Option("Салон"), ft.dropdown.Option("Расходники / Другое")
        ...
        def save_repair(_):
            try:
                ...
                car_profile["repair_history"].append({
                    "repair_name": in_name.value.strip(), "category": in_cat.value,
                    "odometer": odo, "date": dt_str, "part_name": in_pname.value.strip(),
                    "part_code": in_pcode.value.strip(), "cost": cost, "comment": in_comm.value.strip()
                })
                if odo >= car_profile["odometer"].get("value", 0):
                    car_profile["odometer"] = {"value": odo, "date": dt_str}
                engine.save_data(db_data)
        ...
        adlg = ft.AlertDialog(
            ...
            actions=[
                ft.TextButton("Отмена", on_click=lambda e: [setattr(adlg, "open", False), page.update()]),
    ...
    dlg = ft.AlertDialog(
        ...
        content=ft.Column([
            ft.Button("Добавить работу", icon=ft.Icons.ADD, on_click=lambda e: add_repair_click(), bgcolor=ft.Colors.BLUE_GREY_100, height=38),
            search_field, ft.Divider(height=1, color=ft.Colors.BLACK_12), h_col
        ], horizontal_alignment=ft.CrossAxisAlignment.STRETCH, spacing=10, tight=True, width=550),
        actions=[ft.TextButton("Закрыть", on_click=lambda e: [setattr(dlg, "open", False), page.update()])]
    )
    page.overlay.append(dlg)
            ...
            ft.Row(
                ...
                controls=[
                    ft.IconButton(
                        ...
                        on_click=lambda e: network.auto_export_file_to_telegram(page, show_message)
                    ),
                    ft.IconButton(
                    ...
                    ft.IconButton(
                        ...
                        on_click=lambda e: show_add_car_dialog(page, current_db, refresh_callback)
                    ),
                    ft.IconButton(
                        icon=ft.Icons.EDIT,
                        tooltip="Переименовать авто",
                        on_click=lambda e: show_edit_car_name_dialog(page, current_db, selected_car, refresh_callback)
                    ),
                    ft.IconButton(
                        ft.Icons.DELETE_FOREVER,
                        tooltip="Удалить авто",
                        on_click=lambda e: show_delete_car_dialog(page, current_db, selected_car, refresh_callback),
