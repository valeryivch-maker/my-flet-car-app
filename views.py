                ...
                h_col.controls.append(ft.Container( 
                    content=ft.Row([ 
                        ...
                        ft.Row([ 
                            ...
                            ft.IconButton(ft.Icons.DELETE, icon_color=ft.Colors.RED_400, icon_size=18, hover_color=ft.Colors.RED_100, tooltip='Убавить запись', on_click=make_del()) 
                        ], spacing=0) 
                    ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN), 
...
def create_task_actions(page, db_data, p, t, current_km, rebuild, show_msg):
    ...
    def change_click(_):
        ...
        dlg = ft.AlertDialog(title=ft.Text("Правка регламента"), content=ft.Column([n_in, i_in], tight=True),
...
def show_repair_history_dialog(page, db_data, car_profile, rebuild, show_msg):
    ...
    def add_repair_click(_):
        ...
        adlg = ft.AlertDialog(
            ...
            actions=[
                ft.TextButton("Отмена", on_click=lambda _: [setattr(adlg, "open", False), page.update()]),
...
# --- БЛОК ДИАЛОГОВ УПРАВЛЕНИЯ ПРОФИЛЯМИ АВТОМОБИЛЕЙ ---
            ...
            ft.Row(
                ...
                controls=[
                    ...
                    ft.IconButton(
                        ...
                        on_click=lambda _: __import__("threading").Thread(target=async_mobile_import).start() if os.name != "nt" else __import__("threading").Thread(target=async_pc_import).start()
                    ),
                    analytics_btn,
