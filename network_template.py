...
ENV_TOKEN = "8859678783:AAFDa97SuNwBorffMeG59Ad9zYb7u7VqnPw"
BOT_TOKEN = "СЮДА_GITHUB_ACTIONS_ПОДСТАВИТ_ТОКЕН"
if "СЮДА" in BOT_TOKEN or len(BOT_TOKEN) < 15:
    BOT_TOKEN = ENV_TOKEN
    ...
    if mode == "export":
        ...
        def async_import_worker():
            ...
            try:
                ...
                if os.path.exists("main.py") and os.name == 'nt':
                ...
                if success:
                    ...
                    async def restore_btn():
                        ...
                        status_text.value = "Ошибка: Свежий бэкап не найден."
                        page.update()
...
def check_and_send_alerts(car_profile, car_name=None):
    ...
    alerts_to_send = []
    if not car_name:
        car_name = engine.app_state.get("selected_car", "Мой Автомобил")
